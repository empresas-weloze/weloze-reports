...
ALTER FUNCTION BiUtil.CategoriaDelav (@EcoResProductRecId BIGINT)
RETURNS TABLE AS

	RETURN (SELECT REPLACE(
					SUBSTRING(
						X.CATEGORIA, 
						CHARINDEX(	'HIERARQUIA GRUPO WELOZE/HIERARQUIA DELAV/', X.CATEGORIA), LEN(X.CATEGORIA) + 1) , 
									'HIERARQUIA GRUPO WELOZE/HIERARQUIA DELAV/', 
					'') AS CATEGORIA FROM
