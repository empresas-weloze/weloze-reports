...
ALTER FUNCTION BiUtil.CategoriaWeloze (@EcoResProductRecId BIGINT)
RETURNS TABLE AS

	RETURN (SELECT REPLACE(
					SUBSTRING(
						X.CATEGORIA, 
						CHARINDEX(	'HIERARQUIA GRUPO WELOZE/HIERARQUIA WELOZE PARTS/', X.CATEGORIA), LEN(X.CATEGORIA) + 1) , 
									'HIERARQUIA GRUPO WELOZE/HIERARQUIA WELOZE PARTS/', 
					'') AS CATEGORIA FROM
			...
			WHERE P.RECID = @EcoResProductRecId
			) AS X);
