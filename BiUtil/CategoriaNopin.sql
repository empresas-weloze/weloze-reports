...
ALTER FUNCTION BiUtil.CategoriaNopin (@EcoResProductRecId BIGINT)
RETURNS TABLE AS

	RETURN (SELECT REPLACE(
					SUBSTRING(
						X.CATEGORIA, 
						CHARINDEX(	'HIERARQUIA GRUPO WELOZE/HIERARQUIA NOPIN/', X.CATEGORIA), LEN(X.CATEGORIA) + 1) , 
									'HIERARQUIA GRUPO WELOZE/HIERARQUIA NOPIN/', 
					'') AS CATEGORIA FROM
