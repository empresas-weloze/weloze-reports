...
-- ALTER VIEW [BiView].[OrdemProducao_OMT]  AS 
...
SELECT 
	...
	ID.INVENTCOLORID AS "Dimensão - Cor",
...
FROM PRODTABLE P
	...
	CROSS APPLY BiUtil.TipoReferenciaOrdemProducao (P.INVENTREFTYPE) AS TIPOREFERENCIA
	CROSS APPLY BiUtil.StatusOrdemProducao (P.PRODSTATUS) AS STATUSOP 

WHERE P.DATAAREAID = 'OMT'
