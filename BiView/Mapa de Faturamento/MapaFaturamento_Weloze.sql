...
SELECT 
	...
	CASE 
		...
		(SELECT AMOUNTPERIOD FROM CUSTGOAL WHERE DATAAREAID = X.EMPRESA AND NUM = '99' AND YEAR = X.ANO)

		ELSE SUM(CGL.AMOUNTPERIOD)

	END AS "Valor Planejado",

	SUM(X.VALOR_BRUTO)	AS "Valor Faturado"
...
SELECT 
	...
	LEFT JOIN CUSTGOALLINES CGL ON CGL.DATAAREAID = X.EMPRESA AND CGL.CUSTACCOUNT = X.CODIGOCLIENTE 
	LEFT JOIN CUSTGOAL CG ON CG.RECID = CGL.CUSTGOAL AND CG.YEAR = X.ANO
	JOIN CUSTTABLE CT ON CT.ACCOUNTNUM = X.CODIGOCLIENTE AND CT.DATAAREAID = X.EMPRESA
	JOIN DIRPARTYTABLE PT ON PT.RECID = CT.PARTY

GROUP BY X.ANO, CG.NUM, CG.NAME, X.EMPRESA